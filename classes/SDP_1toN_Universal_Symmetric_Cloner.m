classdef SDP_1toN_Universal_Symmetric_Cloner
    ...
    %   cloner = SDP_1toN_Cloning(states, 3);
    ...
    properties
        ...
        N       % Number of output clones (positive integer)
    end
    
    methods
        function obj = SDP_1toN_Universal_Symmetric_Cloner(N)
            % SDP_1toN_Cloning - Construct a 1->N quantum cloning optimizer
            %
            % SYNTAX:
            %   obj = SDP_1toN_Cloning(states, N)
            %
            % INPUT:
            %   N      - Number of output clones (positive integer)
            %
            % OUTPUT:
            %   obj    - SDP_1toN_Cloning object with initialized Omega matrix
            %
            % DESCRIPTION:
            ...
            % Dimension: 2 (input*) ⊗ 2^N (N outputs)
            dim_total = 2^(N+1);
            ...
            % symmetric informationally complete states
            s1 = [1;0];
            s2 = [1/sqrt(3); sqrt(2/3) ];
            s3 = [1/sqrt(3); exp(4*pi*1i/3)*sqrt(2/3) ];
            s4 = [1/sqrt(3); exp(2*pi*1i/3)*sqrt(2/3) ];
